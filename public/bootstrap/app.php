<?php
require '../vendor/autoload.php';
require_once __DIR__."/../config/Database.php";
require_once __DIR__."/../config/Response.php";
require_once __DIR__."/../app/models/Extension.php";
require_once __DIR__."/../app/models/User.php";
require_once __DIR__."/../app/models/Blog.php";
require_once __DIR__."/../app/models/Payment.php";
require_once __DIR__."/../app/models/Payment.php";
require_once __DIR__."/../app/models/UserBookSubscription.php";
require_once __DIR__."/../app/models/BlogCategories.php";
require_once __DIR__."/../app/models/UserPlan.php";
require_once __DIR__."/../app/models/Plan.php";
require_once __DIR__."/../app/controllers/Controller.php";
require_once __DIR__."/../app/controllers/BlogController.php";
require_once __DIR__."/../app/controllers/UserController.php";
require_once __DIR__."/../app/controllers/SubscriptionController.php";
