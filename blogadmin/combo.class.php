<?php if (!defined('datalist_date_separator')) {
    die('datalist.php not included!');
}
...
class Combo
{
    ...
    public $MatchText;
    public $ListType;
    public $ListBoxHeight;
    public $MultipleSeparator;
    public $RadiosPerLine;
    public $AllowNull;
    public $ApplySelect2;
    public $HTML; // the resulting output HTML code to use
    ...
    public function Render()
    {
        ...
        if (!$this->SelectID) {
            $this->SelectID=str_replace(array('[', ']'), '_', $this->SelectName);
        }
        ...
        if ($this->ListType != 2) {
            ...
            if ($this->ListType==3) {
                $arrSelectedData=explode($this->MultipleSeparator, $this->SelectedData);
            }
            if ($this->ListType==3) {
                $arrSelectedText=explode($this->MultipleSeparator, $this->SelectedText);
            }
            ...
            if ($this->ListType == 3 && strlen($this->MatchText)>0) {
                $this->MatchText=substr($this->MatchText, 0, -1 * strlen($this->MultipleSeparator));
            }
        } else {
            global $Translation;
            $separator = '&nbsp; &nbsp; &nbsp; &nbsp;';
