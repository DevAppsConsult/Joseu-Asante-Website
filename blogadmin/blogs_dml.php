...
function blogs_insert()
{
    ...
    if (function_exists('blogs_before_insert')) {
        $args=array();
        if (!blogs_before_insert($data, getMemberInfo(), $args)) {
            return false;
        }
    }
    ...
    if (function_exists('blogs_after_insert')) {
        ...
        $args=array();
        if (!blogs_after_insert($data, getMemberInfo(), $args)) {
            return $recID;
        }
    }
...
function blogs_update($selected_id)
{
    ...
    if (function_exists('blogs_before_update')) {
        $args=array();
        if (!blogs_before_update($data, getMemberInfo(), $args)) {
            return false;
        }
    }
    ...
    if (function_exists('blogs_after_update')) {
        ...
        $args = array();
        if (!blogs_after_update($data, getMemberInfo(), $args)) {
            return;
        }
    }
...
function blogs_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = '')
{
    ...
    if ($AllowInsert) {
        if (!$selected_id) {
            $templateCode = str_replace('<%%INSERT_BUTTON%%>', '<button type="submit" class="btn btn-success" id="insert" name="insert_x" value="1" onclick="return blogs_validateData();"><i class="glyphicon glyphicon-plus-sign"></i> ' . $Translation['Save New'] . '</button>', $templateCode);
        }
    ...
    if ($selected_id) {
        ...
        if ($dvprint) {
            $templateCode = str_replace('<%%VALUE(category)%%>', safe_html($urow['category']), $templateCode);
        }
        if (!$dvprint) {
        ...
        if (!$dvprint) {
            $templateCode = str_replace('<%%VALUE(tags)%%>', html_attr($row['tags']), $templateCode);
        }
        ...
        if ($dvprint) {
            $templateCode = str_replace('<%%VALUE(posted)%%>', safe_html($urow['posted']), $templateCode);
        }
        if (!$dvprint) {
            $templateCode = str_replace('<%%VALUE(posted)%%>', html_attr($row['posted']), $templateCode);
        }
