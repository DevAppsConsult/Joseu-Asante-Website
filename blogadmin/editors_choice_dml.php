...
function editors_choice_insert()
{
    ...
    if ($data['blog'] == empty_lookup_value) {
        $data['blog'] = '';
    }
    if ($data['blog']== '') {
    ...
    if (function_exists('editors_choice_before_insert')) {
        $args=array();
        if (!editors_choice_before_insert($data, getMemberInfo(), $args)) {
            return false;
        }
    }
    ...
    if (function_exists('editors_choice_after_insert')) {
        ...
        $args=array();
        if (!editors_choice_after_insert($data, getMemberInfo(), $args)) {
            return $recID;
        }
    }
...
function editors_choice_update($selected_id)
{
    ...
    if (function_exists('editors_choice_before_update')) {
        $args=array();
        if (!editors_choice_before_update($data, getMemberInfo(), $args)) {
            return false;
        }
    }
    ...
    if (function_exists('editors_choice_after_update')) {
        ...
        $args = array();
        if (!editors_choice_after_update($data, getMemberInfo(), $args)) {
            return;
        }
    }
...
function editors_choice_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = '')
{
    ...
    if ($AllowInsert) {
        if (!$selected_id) {
            $templateCode = str_replace('<%%INSERT_BUTTON%%>', '<button type="submit" class="btn btn-success" id="insert" name="insert_x" value="1" onclick="return editors_choice_validateData();"><i class="glyphicon glyphicon-plus-sign"></i> ' . $Translation['Save New'] . '</button>', $templateCode);
        }
    ...
    if ($selected_id) {
        ...
        if ($dvprint) {
            $templateCode = str_replace('<%%VALUE(blog)%%>', safe_html($urow['blog']), $templateCode);
        }
        if (!$dvprint) {
            $templateCode = str_replace('<%%VALUE(blog)%%>', html_attr($row['blog']), $templateCode);
        }
    ...
    if ($selected_id) {
        ...
        if ($jdata === false) {
            $jdata = get_defaults('editors_choice');
        }
        $rdata = $row;
    }
    $cache_data = array(
