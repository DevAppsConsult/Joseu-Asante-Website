...
<div class="row">
    ...
    ?>
        ...
        ?><a href="#" onclick="<?php echo $current_table; ?>GetChildrenRecordsList({ Verb: 'reload' }); return false;" class="btn btn-default hspacer-sm vspacer-md"><i class="glyphicon glyphicon-refresh"></i></a><?php
    } ?>
        ...
        foreach ($config['display-fields'] as $fieldIndex => $fieldLabel) {
            ?>
                ...
                ?>
								...
								<?php
            } elseif ($parameters['SortBy'] == $fieldIndex && $parameters['SortDirection'] == 'asc') {
                ?>
            ...
            ?>
                ...
                ?>
                    ...
                    ?>
						...
						<td class="<?php echo "{$parameters['ChildTable']}-{$config['display-field-names'][3]}"; ?>" id="<?php echo "{$parameters['ChildTable']}-{$config['display-field-names'][3]}-" . html_attr($record[$config['child-primary-key-index']]); ?>"><img src="thumbnail.php?i=<?php echo $record[3]; ?>&t=titles&f=icon&v=tv" class="img-thumbnail"></td>
						...
						<td class="<?php echo "{$parameters['ChildTable']}-{$config['display-field-names'][12]}"; ?>" id="<?php echo "{$parameters['ChildTable']}-{$config['display-field-names'][12]}-" . html_attr($record[$config['child-primary-key-index']]); ?>"><a href="mailto:<?php echo isEmail($record[12]); ?>" class="btn btn-info" title="<?php echo html_attr($record[12]); ?>"><i class="glyphicon glyphicon-envelope"></i></a></td>
						...
						<td class="<?php echo "{$parameters['ChildTable']}-{$config['display-field-names'][14]}"; ?>" id="<?php echo "{$parameters['ChildTable']}-{$config['display-field-names'][14]}-" . html_attr($record[$config['child-primary-key-index']]); ?>"><img src="<?php echo thisOr(get_embed('googlemap', $record[14], '', '', 'thumbnail_url'), 'blank.gif'); ?>" style="max-width: 50px; max-height: 50px; "></td>
					</tr>
					<?php
        }
    } ?>
