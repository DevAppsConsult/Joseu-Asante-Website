...
function banner_posts_insert()
{
    ...
    if (function_exists('banner_posts_before_insert')) {
        $args=array();
        if (!banner_posts_before_insert($data, getMemberInfo(), $args)) {
            return false;
        }
    }
    ...
    if (function_exists('banner_posts_after_insert')) {
        ...
        $args=array();
        if (!banner_posts_after_insert($data, getMemberInfo(), $args)) {
            return $recID;
        }
    }
...
function banner_posts_update($selected_id)
{
    ...
    if (function_exists('banner_posts_before_update')) {
        $args=array();
        if (!banner_posts_before_update($data, getMemberInfo(), $args)) {
            return false;
        }
    }
    ...
    if (function_exists('banner_posts_after_update')) {
        ...
        $args = array();
        if (!banner_posts_after_update($data, getMemberInfo(), $args)) {
            return;
        }
    }
...
function banner_posts_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = '')
{
    ...
    if ($AllowInsert) {
        if (!$selected_id) {
            $templateCode = str_replace('<%%INSERT_BUTTON%%>', '<button type="submit" class="btn btn-success" id="insert" name="insert_x" value="1" onclick="return banner_posts_validateData();"><i class="glyphicon glyphicon-plus-sign"></i> ' . $Translation['Save New'] . '</button>', $templateCode);
        }
    ...
    if ($selected_id) {
        ...
        if ($jdata === false) {
            $jdata = get_defaults('banner_posts');
        }
        $rdata = $row;
    }
    $cache_data = array(
