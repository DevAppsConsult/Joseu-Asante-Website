...
*/
    ...
    if ($field['list_type'] == 2) {
        ...
        if (!preg_match('/ limit .+/i', $combo->Query)) {
            if (!$search_id) {
                $combo->Query .= " LIMIT {$skip}, {$results_per_page}";
            }
            if ($search_id) {
                $combo->Query .= " LIMIT 1";
            }
        ...
        if (!$search_id && $selected_text) {
            ...
            if ($search_id) {
                $prepared_data[] = array('id' => $search_id, 'text' => $xss->xss_clean($selected_text));
            }
        } else {
        ...
        if (empty($prepared_data)) {
            $prepared_data[] = array('id' => '', 'text' => $Translation['No matches found!']);
        }
