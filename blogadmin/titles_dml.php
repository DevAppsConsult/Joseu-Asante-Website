...
function titles_insert()
{
    ...
    if ($data['website_name'] == empty_lookup_value) {
        $data['website_name'] = '';
    }
    $data['tagline'] = makeSafe($_REQUEST['tagline']);
    if ($data['tagline'] == empty_lookup_value) {
        $data['tagline'] = '';
    }
    ...
    if ($data['googlemap'] == empty_lookup_value) {
        $data['googlemap'] = '';
    }
    $data['icon'] = PrepareUploadedFile('icon', 3072000, 'jpg|jpeg|gif|png', false, '');
    if ($data['icon']) {
        createThumbnail($data['icon'], getThumbnailSpecs('titles', 'icon', 'tv'));
    }
    if ($data['icon']) {
        createThumbnail($data['icon'], getThumbnailSpecs('titles', 'icon', 'dv'));
    }
    ...
    if (function_exists('titles_before_insert')) {
        $args=array();
        if (!titles_before_insert($data, getMemberInfo(), $args)) {
            return false;
        }
    }
    ...
    if (function_exists('titles_after_insert')) {
        ...
        $args=array();
        if (!titles_after_insert($data, getMemberInfo(), $args)) {
            return $recID;
        }
    }
...
function titles_update($selected_id)
{
    ...
    if (function_exists('titles_before_update')) {
        $args=array();
        if (!titles_before_update($data, getMemberInfo(), $args)) {
            return false;
        }
    }
    ...
    if (function_exists('titles_after_update')) {
        ...
        $args = array();
        if (!titles_after_update($data, getMemberInfo(), $args)) {
            return;
        }
    }
...
function titles_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = '')
{
    ...
    if ($AllowInsert) {
        if (!$selected_id) {
            $templateCode = str_replace('<%%INSERT_BUTTON%%>', '<button type="submit" class="btn btn-success" id="insert" name="insert_x" value="1" onclick="return titles_validateData();"><i class="glyphicon glyphicon-plus-sign"></i> ' . $Translation['Save New'] . '</button>', $templateCode);
        }
    ...
    if ($selected_id) {
        ...
        if ($dvprint) {
            $templateCode = str_replace('<%%VALUE(website_name)%%>', safe_html($urow['website_name']), $templateCode);
        }
        if (!$dvprint) {
            $templateCode = str_replace('<%%VALUE(website_name)%%>', html_attr($row['website_name']), $templateCode);
        }
        $templateCode = str_replace('<%%URLVALUE(website_name)%%>', urlencode($urow['website_name']), $templateCode);
        if ($dvprint) {
            $templateCode = str_replace('<%%VALUE(tagline)%%>', safe_html($urow['tagline']), $templateCode);
        }
        if (!$dvprint) {
            $templateCode = str_replace('<%%VALUE(tagline)%%>', html_attr($row['tagline']), $templateCode);
        }
        $templateCode = str_replace('<%%URLVALUE(tagline)%%>', urlencode($urow['tagline']), $templateCode);
        $row['icon'] = ($row['icon'] != '' ? $row['icon'] : 'blank.gif');
        if ($dvprint) {
            $templateCode = str_replace('<%%VALUE(icon)%%>', safe_html($urow['icon']), $templateCode);
        }
        if (!$dvprint) {
            $templateCode = str_replace('<%%VALUE(icon)%%>', html_attr($row['icon']), $templateCode);
        }
        $templateCode = str_replace('<%%URLVALUE(icon)%%>', urlencode($urow['icon']), $templateCode);
        if ($dvprint) {
            $templateCode = str_replace('<%%VALUE(keywords)%%>', safe_html($urow['keywords']), $templateCode);
        }
        if (!$dvprint) {
            $templateCode = str_replace('<%%VALUE(keywords)%%>', html_attr($row['keywords']), $templateCode);
        }
        $templateCode = str_replace('<%%URLVALUE(keywords)%%>', urlencode($urow['keywords']), $templateCode);
        if ($dvprint) {
            $templateCode = str_replace('<%%VALUE(short_description)%%>', safe_html($urow['short_description']), $templateCode);
        }
        if (!$dvprint) {
            $templateCode = str_replace('<%%VALUE(short_description)%%>', html_attr($row['short_description']), $templateCode);
        }
        ...
        if ($dvprint) {
            $templateCode = str_replace('<%%VALUE(bannertext1)%%>', safe_html($urow['bannertext1']), $templateCode);
        }
        if (!$dvprint) {
            $templateCode = str_replace('<%%VALUE(bannertext1)%%>', html_attr($row['bannertext1']), $templateCode);
        }
        $templateCode = str_replace('<%%URLVALUE(bannertext1)%%>', urlencode($urow['bannertext1']), $templateCode);
        if ($dvprint) {
            $templateCode = str_replace('<%%VALUE(bannertext2)%%>', safe_html($urow['bannertext2']), $templateCode);
        }
        if (!$dvprint) {
            $templateCode = str_replace('<%%VALUE(bannertext2)%%>', html_attr($row['bannertext2']), $templateCode);
        }
        $templateCode = str_replace('<%%URLVALUE(bannertext2)%%>', urlencode($urow['bannertext2']), $templateCode);
        if ($dvprint) {
            $templateCode = str_replace('<%%VALUE(bannertext3)%%>', safe_html($urow['bannertext3']), $templateCode);
        }
        if (!$dvprint) {
            $templateCode = str_replace('<%%VALUE(bannertext3)%%>', html_attr($row['bannertext3']), $templateCode);
        }
        $templateCode = str_replace('<%%URLVALUE(bannertext3)%%>', urlencode($urow['bannertext3']), $templateCode);
        if ($dvprint) {
            $templateCode = str_replace('<%%VALUE(bannertext4)%%>', safe_html($urow['bannertext4']), $templateCode);
        }
        if (!$dvprint) {
            $templateCode = str_replace('<%%VALUE(bannertext4)%%>', html_attr($row['bannertext4']), $templateCode);
        }
        ...
        if ($dvprint) {
            $templateCode = str_replace('<%%VALUE(phone)%%>', safe_html($urow['phone']), $templateCode);
        }
        if (!$dvprint) {
