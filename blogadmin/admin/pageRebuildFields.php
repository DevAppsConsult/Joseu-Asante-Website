<?php
    ...
    */
    ...
    {
        if (!isset($schema[$fix_table][$fix_field])) {
            return 0;
        }
    ...
    if ($fix_field && $fix_table) {
        $fix_status = fix_field($fix_table, $fix_field, $schema, $qry);
    }
    ...
    if ($fix_all) {
        foreach ($schema as $tn => $fields) {
            foreach ($fields as $fn => $fd) {
                if (prepare_def($fd['appgini']) == prepare_def($fd['db'])) {
                    continue;
                }
