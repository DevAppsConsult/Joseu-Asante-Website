<?php
    ...
    */
    ...
    function get_tables_info($skip_authentication = false)
    {
        ...
        if (($skip_authentication || getLoggedAdmin()) && count($all_tables)) {
            return $all_tables;
        }
        if (!$skip_authentication && count($accessible_tables)) {
        ...
        $all_tables = array(
            /* ['table_name' => [table props assoc array] */
                'titles' => array(
                    ...
                    'Description' => '',
                    'tableIcon' => 'resources/table_icons/drugs_com.png',
                    'group' => $tg[0],
                    'homepageShowCount' => 0
                ),
                'links' => array(
                    'Caption' => 'Links',
                    'Description' => '',
                    'tableIcon' => 'resources/table_icons/firefox.png',
                    'group' => $tg[0],
                    'homepageShowCount' => 0
                ),
                'blog_categories' => array(
                    ...
                    'Description' => '',
                    'tableIcon' => 'resources/table_icons/award_star_bronze_2.png',
                    'group' => $tg[0],
                    'homepageShowCount' => 0
                ),
                'blogs' => array(
                    'Caption' => 'Blogs',
                    'Description' => '',
                    'tableIcon' => 'resources/table_icons/feed.png',
                    'group' => $tg[0],
                    'homepageShowCount' => 0
                ),
                'editors_choice' => array(
                    'Caption' => 'Editors choice',
                    'Description' => '',
                    'tableIcon' => 'resources/table_icons/bullet_star.png',
                    'group' => $tg[0],
                    'homepageShowCount' => 0
                )
        );
    ...
    {
        if ($tableName=='titles' && $fieldName=='icon' && $view=='tv') {
            return array('width'=>50, 'height'=>50, 'identifier'=>'_tv');
        } elseif ($tableName=='titles' && $fieldName=='icon' && $view=='dv') {
            return array('width'=>250, 'height'=>250, 'identifier'=>'_dv');
        } elseif ($tableName=='blogs' && $fieldName=='photo' && $view=='tv') {
            return array('width'=>50, 'height'=>50, 'identifier'=>'_tv');
        } elseif ($tableName=='blogs' && $fieldName=='photo' && $view=='dv') {
            return array('width'=>250, 'height'=>250, 'identifier'=>'_dv');
        }
        return false;
    }
        ...
        if (is_array($arrValue)) {
            for ($i = 0; $i < count($arrValue); $i++) {
                ...
                if ($arrValue[$i] == $selectedValue) {
                    $selected = "selected class=\"{$selectedClass}\"";
                }
    ...
    function setupMembership()
    {
        ...
        $res = sql(
            ...
            "order by g.groupID, gp.tableName",
            $eo
        );
        while ($row = db_fetch_assoc($res)) {
    ...
    function application_url($page = '', $s = false)
    {
        ...
        if (substr($uri, -12, 12) == '/admin/admin') {
            $uri = substr($uri, 0, -6);
        } elseif (substr($uri, -6, 6) == '/admin' && !$app_folder_is_admin) {
            $uri = substr($uri, 0, -6);
        } elseif ($uri == '/' || $uri == '\\') {
            $uri = '';
        }
    ...
    */
    ...
    function get_plugins()
    {
        ...
        while (false !== ($plugin = $pd->read())) {
            if (!is_dir($plugins_path . $plugin) || in_array($plugin, array('projects', 'plugins-resources', '.', '..'))) {
                continue;
            }
    ...
    function html_attr($str)
    {
        if (version_compare(PHP_VERSION, '5.2.3') >= 0) {
            return htmlspecialchars($str, ENT_QUOTES, datalist_db_encoding, false);
        }
    ...
    class Request
    {
        ...
        public function Request($var, $filter = false)
        {
        ...
        */
        ...
        public static function placeholder()
        {
            if (self::$placeholder_id) {
                return '';
            } // output placeholder code only once
        ...
        protected static function default_options(&$options)
        {
            if (!isset($options['message'])) {
                $options['message'] = 'Notification::show() called without a message!';
            }
            ...
            if (!isset($options['dismiss_seconds']) || isset($options['dismiss_days'])) {
                $options['dismiss_seconds'] = 0;
            }

            if (!isset($options['dismiss_days'])) {
    ...
    function sendmail($mail)
    {
        ...
        if (!class_exists('PHPMailer')) {
            ...
            if ($smtp) {
                include("{$curr_dir}/../resources/PHPMailer/class.smtp.php");
            }
        }
        ...
        if (function_exists('sendmail_handler')) {
            sendmail_handler($pm);
        }
