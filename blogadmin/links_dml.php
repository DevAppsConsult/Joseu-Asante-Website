...
function links_insert()
{
    ...
    if ($data['facebook'] == empty_lookup_value) {
        $data['facebook'] = '';
    }
    $data['twitter'] = makeSafe($_REQUEST['twitter']);
    if ($data['twitter'] == empty_lookup_value) {
        $data['twitter'] = '';
    }
    $data['googleplus'] = makeSafe($_REQUEST['googleplus']);
    if ($data['googleplus'] == empty_lookup_value) {
        $data['googleplus'] = '';
    }
    $data['pinterest'] = makeSafe($_REQUEST['pinterest']);
    if ($data['pinterest'] == empty_lookup_value) {
        $data['pinterest'] = '';
    }
    $data['dribble'] = makeSafe($_REQUEST['dribble']);
    if ($data['dribble'] == empty_lookup_value) {
        $data['dribble'] = '';
    }
    ...
    if (function_exists('links_before_insert')) {
        $args=array();
        if (!links_before_insert($data, getMemberInfo(), $args)) {
            return false;
        }
    }
    ...
    if (function_exists('links_after_insert')) {
        ...
        $args=array();
        if (!links_after_insert($data, getMemberInfo(), $args)) {
            return $recID;
        }
    }
...
function links_update($selected_id)
{
    ...
    if (function_exists('links_before_update')) {
        $args=array();
        if (!links_before_update($data, getMemberInfo(), $args)) {
            return false;
        }
    }
    ...
    if (function_exists('links_after_update')) {
        ...
        $args = array();
        if (!links_after_update($data, getMemberInfo(), $args)) {
            return;
        }
    }
...
function links_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = '')
{
    ...
    if ($AllowInsert) {
        if (!$selected_id) {
            $templateCode = str_replace('<%%INSERT_BUTTON%%>', '<button type="submit" class="btn btn-success" id="insert" name="insert_x" value="1" onclick="return links_validateData();"><i class="glyphicon glyphicon-plus-sign"></i> ' . $Translation['Save New'] . '</button>', $templateCode);
        }
    ...
    if ($selected_id) {
        ...
        if ($dvprint) {
            $templateCode = str_replace('<%%VALUE(facebook)%%>', safe_html($urow['facebook']), $templateCode);
        }
        if (!$dvprint) {
            $templateCode = str_replace('<%%VALUE(facebook)%%>', html_attr($row['facebook']), $templateCode);
        }
        $templateCode = str_replace('<%%URLVALUE(facebook)%%>', urlencode($urow['facebook']), $templateCode);
        if ($dvprint) {
            $templateCode = str_replace('<%%VALUE(twitter)%%>', safe_html($urow['twitter']), $templateCode);
        }
        if (!$dvprint) {
            $templateCode = str_replace('<%%VALUE(twitter)%%>', html_attr($row['twitter']), $templateCode);
        }
        $templateCode = str_replace('<%%URLVALUE(twitter)%%>', urlencode($urow['twitter']), $templateCode);
        if ($dvprint) {
            $templateCode = str_replace('<%%VALUE(googleplus)%%>', safe_html($urow['googleplus']), $templateCode);
        }
        if (!$dvprint) {
            $templateCode = str_replace('<%%VALUE(googleplus)%%>', html_attr($row['googleplus']), $templateCode);
        }
        $templateCode = str_replace('<%%URLVALUE(googleplus)%%>', urlencode($urow['googleplus']), $templateCode);
        if ($dvprint) {
            $templateCode = str_replace('<%%VALUE(pinterest)%%>', safe_html($urow['pinterest']), $templateCode);
        }
        if (!$dvprint) {
            $templateCode = str_replace('<%%VALUE(pinterest)%%>', html_attr($row['pinterest']), $templateCode);
        }
        $templateCode = str_replace('<%%URLVALUE(pinterest)%%>', urlencode($urow['pinterest']), $templateCode);
        if ($dvprint) {
            $templateCode = str_replace('<%%VALUE(dribble)%%>', safe_html($urow['dribble']), $templateCode);
        }
        if (!$dvprint) {
            $templateCode = str_replace('<%%VALUE(dribble)%%>', html_attr($row['dribble']), $templateCode);
        }
