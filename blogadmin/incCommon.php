<?php
    ...
    */
    ...
    function getTableList($skip_authentication = false)
    {
        ...
        $arrTables = array(
            ...
            'titles' => array('Website Details', '', 'resources/table_icons/drugs_com.png', 'None'),
            'links' => array('Links', '', 'resources/table_icons/firefox.png', 'None'),
            'blog_categories' => array('Blog categories', '', 'resources/table_icons/award_star_bronze_2.png', 'None'),
            'blogs' => array('Blogs', '', 'resources/table_icons/feed.png', 'None'),
            'editors_choice' => array('Editors choice', '', 'resources/table_icons/bullet_star.png', 'None')
        );
    ...
    {
        $sql_fields = array(
            'titles' => "`titles`.`id` as 'id', `titles`.`website_name` as 'website_name', `titles`.`tagline` as 'tagline', `titles`.`icon` as 'icon', `titles`.`keywords` as 'keywords', `titles`.`short_description` as 'short_description', `titles`.`detailed_description` as 'detailed_description', `titles`.`bannertext1` as 'bannertext1', `titles`.`bannertext2` as 'bannertext2', `titles`.`bannertext3` as 'bannertext3', `titles`.`bannertext4` as 'bannertext4', `titles`.`address` as 'address', `titles`.`email` as 'email', `titles`.`phone` as 'phone', `titles`.`googlemap` as 'googlemap'",
            'links' => "`links`.`id` as 'id', `links`.`facebook` as 'facebook', `links`.`twitter` as 'twitter', `links`.`googleplus` as 'googleplus', `links`.`pinterest` as 'pinterest', `links`.`dribble` as 'dribble', `links`.`comments_script` as 'comments_script', `links`.`sharing_script` as 'sharing_script', `links`.`javascript` as 'javascript'",
            'blog_categories' => "`blog_categories`.`id` as 'id', `blog_categories`.`name` as 'name'",
            'blogs' => "`blogs`.`id` as 'id', `blogs`.`title` as 'title', IF(    CHAR_LENGTH(`blog_categories1`.`name`), CONCAT_WS('',   `blog_categories1`.`name`), '') as 'category', `blogs`.`tags` as 'tags', `blogs`.`content` as 'content', `blogs`.`photo` as 'photo', if(`blogs`.`date`,date_format(`blogs`.`date`,'%m/%d/%Y'),'') as 'date', `blogs`.`author` as 'author', `blogs`.`posted` as 'posted'",
            'editors_choice' => "`editors_choice`.`id` as 'id', IF(    CHAR_LENGTH(`blogs1`.`title`) || CHAR_LENGTH(`blogs1`.`author`), CONCAT_WS('',   `blogs1`.`title`, ' :author ', `blogs1`.`author`), '') as 'blog'"
        );
    ...
    function get_sql_from($table_name, $skip_permissions = false)
    {
        $sql_from = array(
            ...
            'blogs' => "`blogs` LEFT JOIN `blog_categories` as blog_categories1 ON `blog_categories1`.`id`=`blogs`.`category` ",
            'editors_choice' => "`editors_choice` LEFT JOIN `blogs` as blogs1 ON `blogs1`.`id`=`editors_choice`.`blog` "
        );
    ...
    function logInMember()
    {
        ...
        if ($_POST['signIn'] != '') {
            ...
            exit;
        } elseif ((!$_SESSION['memberID'] || $_SESSION['memberID']==$adminConfig['anonymousMember']) && $_COOKIE['BLOG_ADMIN_rememberMe']!='') {
    ...
    function htmlUserBar()
    {
		...
		<nav class="navbar navbar-default navbar-fixed-top hidden-print" role="navigation">
			<div class="navbar-header">
				...
				<a class="navbar-brand" href="<?php echo PREPEND_PATH; ?>index.php"><i class="glyphicon glyphicon-home"></i> <img src="kashipara.png" height="50" width="100"></a>
			</div>
    ...
    {
        // validate input
        if ($index < 1 || $index > 80 || !is_int($index)) {
            return false;
        }
        if ($filterAnd != 'or') {
            $filterAnd = 'and';
    ...
    */
    ...
    function filterDropdownBy($filterable, $filterers, $parentFilterers, $parentPKField, $parentCaption, $parentTable, &$filterableCombo)
    {
        ...
        foreach ($filterersArray as $filterer) {
            if (is_array($filterableDataByFilterer[$filterer])) {
                foreach ($filterableDataByFilterer[$filterer] as $filtererItem => $filterableItem) {
    ...
    function get_embed($provider, $url, $max_width = '', $max_height = '', $retrieve = 'html')
    {
        ...
        if ($providers[$provider]['oembed']) {
            ...
            if ($data === null) {
                ...
                if ($retrieve == 'html') {
                    return "<div class=\"text-danger\">{$data_json}\n<!-- {$oembed} --></div>";
                }
                return '';
            }
        ...
        if ($provider == 'googlemap') {
            return get_embed_googlemap($url, $max_width, $max_height, $retrieve);
        }
    ...
    function NavMenus($options = array())
    {
        ...
        if ((count($table_group_name) == 1 && $table_group_name[0] == 'None') || count($table_group_name) < 1) {
...
EOT;
    ...
    {
        if (!is_array($homeLinks) || !count($homeLinks)) {
            return '';
        }
        ...
        foreach ($homeLinks as $link) {
            ...
            if ($tgroup != $link['table_group'] && $tgroup != '*') {
                continue;
            }

            /* fall-back classes if none defined */
            if (!$link['grid_column_classes']) {
                $link['grid_column_classes'] = $default_classes['grid_column'];
            }
            if (!$link['panel_classes']) {
                $link['panel_classes'] = $default_classes['panel'];
            }
            if (!$link['link_classes']) {
                $link['link_classes'] = $default_classes['link'];
            }
