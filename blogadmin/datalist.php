...
class DataList
{
    ...
    public $QueryFieldsTV;
    public $QueryFieldsCSV;
    public $QueryFieldsFilters;
    public $QueryFieldsQS;
    public $QueryFrom;
    public $QueryWhere;
    public $QueryOrder;
    public $filterers;
    ...
    public $AllowDelete;
    public $AllowMassDelete;
    public $AllowDeleteOfParents;
    public $AllowInsert;
    public $AllowUpdate;
    public $SeparateDV;
    public $Permissions;
    public $AllowFilters;
    public $AllowSavingFilters;
    public $AllowSorting;
    public $AllowNavigation;
    public $AllowPrinting;
    public $HideTableView;
    public $AllowCSV;
    public $CSVSeparator;
    public $QuickSearch;
    public $RecordsPerPage;
    public $ScriptFileName;
    public $RedirectAfterInsert;
    public $TableTitle;
    public $PrimaryKey;
    ...
    public $TemplateDV;
    public $TemplateDVP;
    public $ShowTableHeader;
    public $ShowRecordSlots;
    public $TVClasses;
    public $DVClasses;
    public $ContentType;
    public $HTML;           // generated html after calling Render()
    ...
    public function Render()
    {
        ...
        if ($AutoClose) {
            $this->HTML .= '<input name="AutoClose" value="1" type="hidden">';
        }
        $this->HTML .= '<script>';
        ...
        if (!$this->HideTableView && !($dvprint_x && $this->AllowSelection && $SelectedID) && !$PrintDV && !$Embedded) {
            ...
            $this->HTML .= '<thead><tr>';
            if (!$Print_x) {
                $this->HTML .= '<th style="width: 18px;" class="text-center"><input class="hidden-print" type="checkbox" title="' . html_attr($Translation['Select all records']) . '" id="select_all_records"></th>';
            }
            // Templates
            ...
            if ($RecordCount) {
                ...
                while (($row = db_fetch_array($result)) && ($i < ($FirstRecord + $this->RecordsPerPage))) {
                    ...
                    if (($PrintTV || $Print_x) && count($_REQUEST['record_selector']) && !in_array($row[$FieldCountTV], $_REQUEST['record_selector'])) {
                        continue;
                    }
            ...
            if ($Print_x == '') { // TV
                ...
                if ($RecordCount > $this->RecordsPerPage) {
                    ...
                    $pagesMenu = $Translation['go to page'] . ' <select style="width: 90%; max-width: 8em;" class="input-sm ltr" id="' . $pagesMenuId . '" onChange="document.myform.writeAttribute(\'novalidate\', \'novalidate\'); document.myform.NoDV.value=1; document.myform.FirstRecord.value=(this.value * ' . $this->RecordsPerPage . '+1); document.myform.submit();">';
                    $pagesMenu .= '</select>';
            ...
            if ($SearchString!='') {
                $this->HTML .= '<script>$j(function(){ $j(".table-responsive td").mark("' . html_attr($SearchString) . '", { className: "text-warning bg-warning", diacritics: false }); })</script>';
            }
        ...
        if ($this->QuickSearch && !strpos($this->HTML, 'SearchString')) {
            $this->HTML .= '<input name="SearchString" type="hidden" value="' . html_attr($SearchString) . '">';
        }
        ...
        if ($PrintDV) {
            ...
            $selectedRecords = 0;
            if (is_array($_REQUEST['record_selector'])) {
    ...
    public function validate_filters($req, $FiltersPerGroup = 4, $is_gpc = true)
    {
        ...
        for ($i = 1; $i <= datalist_filters_count * $FiltersPerGroup; $i++) {
            if (!isset($fand[$i]) && !isset($ffield[$i]) && !isset($fop[$i]) && !isset($fvalue[$i])) {
                continue;
            }
     ...
     */
    public function tv_tools()
    {
        global $Translation;

        ob_start(); ?>
		<div class="pull-right flip btn-group vspacer-md" id="tv-tools">
